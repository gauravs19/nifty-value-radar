...
def _chat_ids(chat_id):
    """TELEGRAM_CHAT_ID may be a single id or a comma-separated list, to fan
    the same report out to multiple recipients."""
    raw = chat_id or os.environ.get("TELEGRAM_CHAT_ID") or ""
    ...
    for cid in chat_ids:
        for i in range(0, len(text), MAX_LEN):
            ...
            if resp.status_code != 200:
                print(f"Telegram send failed for chat {cid}: {resp.status_code} {resp.text}")
    ...
    for cid in chat_ids:
        with open(file_path, "rb") as f:
            resp = requests.post(
                ...
                data={"chat_id": cid, "caption": (caption or "")[:1024]},
                files={"document": (filename or os.path.basename(file_path), f, "text/html")},
                timeout=30,
            )
        if resp.status_code != 200:
            print(f"Telegram document send failed for chat {cid}: {resp.status_code} {resp.text}")
